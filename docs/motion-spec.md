...
| **Tabela como narrativa** | O lado "software sob encomenda" é riscado e o lado Xiax entra maior, com o quadrado da marca, dirigido pelo scroll. | scroll | reduced-motion: estático |
| **Método em sequência** | Símbolo fixo ao lado dos quatro passos: um satélite preenche por passo lido (loader "carregando em sequência"). | scroll (IntersectionObserver) | — (é estado, não movimento) |
| **Frentes assentam** | Os quatro cantos deslizam 28 px do centro para os cantos ao entrar na tela (≥ 64 rem, `animation-timeline: view()`). | scroll | navegador sem suporte ou reduced-motion → estático |
| **Mosaico das frentes** | O núcleo da seção é uma grade fixa 10×10: 40 células violetas (transparentes — o núcleo viajante as pinta quando pousa) e 60 brancas, embaralhadas com semente fixa. Placa branca no meio (linhas 3–6, colunas 1–8) com o texto em tinta, dimensionado em `cqw`. Não anima: a animação é o núcleo chegando. | scroll (núcleo) | — |
...
| **Faixa de polaridade** | "Donos do motor" inverte os tokens só naquele bloco: papel branco no site escuro. O núcleo viajante continua visível por trás. | — | — |
...
| **Destaque violeta** | Título e apoio do contato e a etiqueta do card: o violeta varre as palavras conforme a página rola (`@property --sweep` no bloco, `background-size` no texto), e as letras escurecem por onde ele passou. Sem linha do tempo de scroll, `SweepDriver` faz o mesmo em JS. | scroll | reduced-motion: coberto e parado |
| **O núcleo termina no botão** | O último encaixe do núcleo viajante é o botão "Enviar mensagem": o violeta que se seguiu vira a ação. O card do formulário não tem fundo, para o núcleo aparecer por trás. | scroll | reduced-motion: botão violeta parado |
...
(smooth scroll de biblioteca). Sombra. Cor fora do núcleo — com a exceção que o próprio manual
...
|---|---|---|
| Cabeçalho | três itens como link; em 320 px, no tamanho de dado | "Contato" como botão de contorno |
| Hero | símbolo antes do título; barra fixa com a ação principal no rodapé da tela, que some quando o formulário está visível | idem, símbolo maior |
| Serviço | tabela vira pares empilhados com rótulo em mono | tabela normal |
...
| Contato | a frase quebra por linha; botão vira o encaixe final do núcleo | idem |
