...
O site SHALL rodar em Docker (build multi-stage, saída `standalone` do Next.js) atrás do Caddy, que emite o certificado TLS para `xiax.com.br` e `www`. `docker compose up -d --build` sobe tudo; `scripts/deploy.sh` atualiza depois de um `git pull`.
...
`NEXT_PUBLIC_SITE_URL` (padrão `https://xiax.com.br`) SHALL alimentar URLs canônicas, sitemap, robots e JSON-LD; `NEXT_PUBLIC_CONTACT_EMAIL` e `CONTACT_WEBHOOK_URL` são opcionais e validadas em `src/lib/env.ts`. `.env` fica fora do git.
